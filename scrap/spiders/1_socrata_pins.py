...
class ScrapSpider(CSVFeedSpider):
    ...
    """
    name = '1_socrata_pins'                     # 1 means 1-level.
    allowed_domains = ['ccrecorder.org']
    start_urls = ['https://datacatalog.cookcountyil.gov/resource/6gsb-287d.csv?$where=pina=14&pinsa=05&pinb=210']        # the list of PINs to be verified.
    headers = ['objectid', 'name', 'the_geom', 'pin10',
               'pina', 'pinsa', 'pinb', 'pinp', 'pinu',
               'pinac', 'taxcode', 'job_no', 'upper_elev',
               'lower_elev', 'survey_cal', 'parceltype',
    ...
    def parse_row(self, response, row):
        ...
        pin = row['pin10']                                                # the name of the column defined in 'headers'
        if not pin.startswith('pin10'):
